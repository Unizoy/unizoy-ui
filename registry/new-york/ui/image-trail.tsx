...
export const ImageTrail: React.FC<ImageTrailProps> = ({
    ...
    setActiveImages((prev) => {
      ...
      const updatedImages = prev.map(img => {
        if (now - img.timestamp > timeToRemove && !img.isExpiring) {
          ...
          const imgElement = imagesRef.current?.querySelector(`[data-id="${img.id}"]`);
      ...
      return updatedImages.filter(img => now - img.timestamp < duration*1000);
    });
  }, [duration,fadeOutDuration]);
  ...
  useGSAP(() => {
    ...
    const prevImage = activeImages.length > 1 ? activeImages[activeImages.length - 2] : null;
  ...
  const addImage = (x: number, y: number) => {
    ...
    const distance = getDistance(x, y, lastPositionRef.current.x, lastPositionRef.current.y);
    ...
    setActiveImages((prev) => {
      ...
      if (newImages.length > MAX_IMAGES) {
        ...
        if (!oldestImage.isExpiring) {
          ...
          // setTimeout(() => {
            const imgElement = imagesRef.current?.querySelector(`[data-id="${oldestImage.id}"]`);
            if (imgElement) {
              imgElement.classList.add('fading');
              gsap.to(imgElement, {
                opacity: 0,
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center">
        <h1 className="text-6xl font-bold text-white mix-blend-difference">{text}</h1>
      </div>
    </div>
  );
};
