  ...
  useEffect(() => {
    ...
    if (previousIndex.current !== null) {
      ...
      if (prevTooltip) {
        ...
        activeAnimations.current[previousIndex.current] = gsap.fromTo(prevTooltip,
          {
            opacity: 0.8,
            y: 5,
            scale:0.8
      ...
      {items.map((item) => (
        ...
        >
          ...
          >
            ...
            <div className="font-bold text-white relative z-30 text-base text-center mx-auto">{item.name}</div>
            <div className="text-white text-xs text-center mx-auto">{item.designation}</div>
          </div>
