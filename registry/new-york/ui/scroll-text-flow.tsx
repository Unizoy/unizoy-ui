...
const ScrollTextFlow: React.FC<ScrollTextFlowProps> = ({
  ...
  }, [slideGroups, scrollTriggerStart, scrollTriggerEnd, useMarkers,scrollerRef]);
  ...
  return (
    <div ref={containerRef} className={cn("flex justify-center h-screen py-5", wrapperClassName)}>
      <div className={cn(" space-y-3 md:space-y-5 mx-auto text-center w-full")}>
        ...
        {slideGroups.map((group, idx) =>
          idx < topGroupCount ? (
            <div key={idx} ref={addToGroupRefs} className={cn("flex justify-center items-center gap-5 md:gap-7 lg:gap-10 flex-nowrap", groupClassName)}>
              {group.map((item, i) => (
                <div
                  key={i}
                  className={cn( "text-[#232323] w-fit h-fit px-1 font-normal",item.style)}
        ...
        <div ref={wordsContainerRef} className={cn("flex justify-center items-center my-5 mb-8 max-w-[60%] mx-auto",textWrapperClassName)}>
