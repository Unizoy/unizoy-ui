...
const ScalingSlidingCards = ({
  ...
  const [percentages, setPercentages] = useState<number[]>(new Array(photos?.length ?? 0).fill(0));
  ...
  const setCardRef = (el: HTMLDivElement | null, index: number) => {
    if (el) {
      ...
      if (!fullDimensionsRef.current[index]) {
        ...
        fullDimensionsRef.current[index] = { width: rect.width, height: rect.height };
        // Set initial dimensions to half size.
        gsap.set(el, { width: rect.width / 2, height: rect.height / 2 });
        setDimensions((prev) => {
          const newDims = [...prev];
          newDims[index] = { width: Math.round(rect.width / 2), height: Math.round(rect.height / 2) };
          return newDims;
        });
      }
    }
      ...
      tl.eventCallback("onUpdate", () => {
        ...
        cardRefs.current.forEach((card, index) => {
          ...
          if (fullDim) {
            ...
            const newWidth = (fullDim.width / 2) + ((fullDim.width / 2) * (clampedPercent / 100));
            const newHeight = (fullDim.height / 2) + ((fullDim.height / 2) * (clampedPercent / 100));
            newDimensions[index] = { width: Math.round(newWidth), height: Math.round(newHeight) };
            gsap.set(card, { width: newWidth, height: newHeight });
          } else {
            newDimensions[index] = { width: Math.round(rect.width), height: Math.round(rect.height) };
          }
        });
  ...
  }, [fromXPercent, toXPercent, triggerStart, triggerEnd, scrubSpeed, useMarkers]);

  return (
    <div
      ref={containerRef}
      className={cn("overflow-x-hidden flex justify-center items-start", wrapperClassName)}
    >
      <div ref={groupRef} style={{ gap: `${gap}px` }} className="flex justify-center items-start">
        {photos?.map((photo, i) => (
          <div key={i} ref={(el) => setCardRef(el, i)} className={cn("relative  border-red-500", photo.style)}>
